...
public class GunRipperWindow : EditorWindow
{
	...
	public string ExportPath = "Assets/Packages/Otherloader/Tools/GunRipper/Export";
	...
	private void CreateExportFolder(string exportFolderPath)
    {
		if (!AssetDatabase.IsValidFolder(exportFolderPath))
		{
			string parentFolder = Path.GetDirectoryName(exportFolderPath);
			string exportFolderName = Path.GetFileName(exportFolderPath);
    ...
    {
		string exportFolderPath = ExportPath.Trim('/');
